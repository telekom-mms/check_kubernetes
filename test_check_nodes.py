    ...
    mock.Mock(
        status=mock.Mock(
            conditions=[
                mock.Mock(type='Ready', status='True'),
                mock.Mock(type='DiskPressure', status='True'),
                mock.Mock(type='PIDPressure', status='True'),
            ],
        )
    ),
    mock.Mock(
        status=mock.Mock(
            conditions=[
                mock.Mock(type='Ready', status='True'),
                mock.Mock(type='DiskPressure', status='False'),
                mock.Mock(type='PIDPressure', status='False'),
            ],
        )
    ),
]
...
class TestCheckNodes(unittest.TestCase):
    ...
    @mock.patch('check_nodes.client.CoreV1Api')
    @mock.patch('check_nodes.config.load_kube_config')
    def test_node_two_problem(self, mock_config, mock_client, mock_metric):
        mock_kube = mock.Mock()

        type(mock_kube.list_node.return_value).items = mock.PropertyMock(return_value=nodes_two_problem)
